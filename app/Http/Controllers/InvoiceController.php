...
class InvoiceController extends \TCG\Voyager\Http\Controllers\Controller
{
    ...
    public function edit(Request $request, $id)
    {
        ...
        $suppliers = Supplier::notcharity()->get();
        $currencies = ['USD','TR'];
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','materials','suppliers','items','currencies'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        for ($index = 0; $index < $old_request_count; $index++) {
            if ($request->input("material_id")[$index]) {
                $olditem = InvoiceItem::find($request->input("id")[$index]);
                if ($olditem) {
                    if ($request->input("quantities")[$index] == 0) {
                        ...
                        $olditem->update([
                            ...
                            'supplier_id' => $request->input("supplier_id")[$index],
                            'receipt_num' => $request->input("receipt_num")[$index],
                            'note' => $request->input("note")[$index],
                            'currency' => $request->input("currency")[$index],
                            'individual_price' => $request->input("individual_price")[$index],
        ...
        for ($index = $old_request_count; $index < count($request->input("material_id")); $index++) {
            if ($request->input("material_id")[$index]) {
                if ($request->input("quantities")[$index] != 0) {
                    ...
                    $item['material_id'] = \request("material_id")[$index];
                    $item['quantities'] = \request("quantities")[$index];
                    $item['supplier_id'] = \request("supplier_id")[$index];
                    $item['receipt_num'] = \request("receipt_num")[$index];
                    $item['note'] = \request("note")[$index];
                    $item['currency'] = \request("currency")[$index];
                    $item['individual_price'] = \request("individual_price")[$index];
    ...
    public function create(Request $request)
    {
        ...
        $suppliers = Supplier::notcharity()->get();
        $currencies = ['USD','TR'];
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','project_id','materials','suppliers','currencies'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $duration = ($project->duration)?abs($project->duration):1;
        $duration_unit = Project::getUnit($project->duration_unit);
        $finish_date = $start_date->add($duration_unit,$duration);
        if ($current_time->gt($finish_date)) {
