...
class ReceiptController extends \TCG\Voyager\Http\Controllers\Controller
{
    ...
    public function checkQuantities(Request $request){
        ...
        $invoiceQty = Project::find($project_id)->invoiceItems->where('material_id' ,'=', $material_id)->sum('quantities');
        $receiptQty = Project::find($project_id)->receiptItems->where('material_id' ,'=', $material_id)->sum('quantities');
        $transferQty = Project::find($project_id)->transferItems->where('material_id' ,'=', $material_id)->sum('quantities');
        $last_item = Project::find($project_id)->invoiceItems->where('material_id' ,'=', $material_id)->last();
        if($last_item){
            return response()->json([
                'avaliable' => ($receiptQty + $transferQty) - $invoiceQty ,
                'price' => $last_item->individual_price,
    ...
    public function edit(Request $request, $id)
    {
        ...
        $project = Receipt::find($id)->project;
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','project','materials','stages','items'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        for ($index = 0; $index < $old_request_count; $index++) {
            if ($request->input("material_id")[$index]) {
                $olditem = ReceiptItem::find($request->input("id")[$index]);
                if ($olditem) {
                    if ($request->input("quantities")[$index] == 0) {
                        ...
                        $olditem->update([
                            ...
                            'stage_id' => $request->input("stage_id")[$index],                            
        ...
        for ($index = $old_request_count; $index < count($request->input("material_id")); $index++) {
            if ($request->input("material_id")[$index]) {
                if ($request->input("quantities")[$index] != 0) {
                    ...
                    $item['stage_id'] = \request("stage_id")[$index];                    
    ...
    public function create(Request $request)
    {
        ...
        $materials = Material::all();
        $stages = Stage::all();
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','project','materials','stages'));
    }
