...
class TransferController extends \TCG\Voyager\Http\Controllers\Controller
{
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            if ($dataType->scope && $dataType->scope != '' && method_exists($model, 'scope'.ucfirst($dataType->scope))) {
                if (\request("project_id")){
                    $query = $model->{$dataType->scope}()->where('project_id','=',$project->id);
                }else{
                ...
                if (\request("project_id")){
                    $query = $model::select('*')->where('project_id','=',$project->id);
                }else{
                    $query = $model::select('*');
                }
    ...
    public function edit(Request $request, $id)
    {
        ...
        $items = TransferItem::where('transfer_id',$id)->get();
        $project = Transfer::find($id)->project;
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','project','materials','items'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $old_request_count =  ($request->has("id")) ? count($request->input("id")) : 0;
        for ($index = 0; $index < $old_request_count; $index++) {
            if ($request->input("material_id")[$index]) {
                $olditem = TransferItem::find($request->input("id")[$index]);
                if ($olditem) {
                    $oldinvoiceItem = InvoiceItem::find($olditem->invoice_item_id);
                    if ($request->input("quantities")[$index] == 0) {                        
                        $oldinvoiceItem->delete();
                        $olditem->delete();
                    } else {
                        $olditem->update([
                            'material_id' => $request->input("material_id")[$index],
                            'quantities' => $request->input("quantities")[$index],
                            'note' => $request->input("note")[$index],                                                      
                        ]);
                        $oldinvoiceItem->update([
                            'material_id' => $request->input("material_id")[$index],
                            'quantities' => $request->input("quantities")[$index],
                            'total_price' => $oldinvoiceItem->individual_price * $request->input("quantities")[$index],                            
                            'note' => $request->input("note")[$index],                                                      
                        ]);
                    }
                }
        ...
        for ($index = $old_request_count; $index < count($request->input("material_id")); $index++) {
            if ($request->input("material_id")[$index]) {
                if ($request->input("quantities")[$index] != 0) {
                    $invoiceItem = new InvoiceItem();
                        $invoiceItem['invoice_id'] = $data->invoice_id;
                        $invoiceItem['material_id'] = \request("material_id")[$index];
                        $invoiceItem['quantities'] = \request("quantities")[$index];
                        $invoiceItem['supplier_id'] = 4;
                        $invoiceItem['receipt_num'] = $data->id;
                        $invoiceItem['individual_price'] = \request("price")[$index];  
                        $invoiceItem['currency'] = \request("currency")[$index];                                       
                        $invoiceItem['total_price'] = \request("price")[$index] * \request("quantities")[$index];                                      
                        $invoiceItem['note'] = \request("note")[$index];                                      
                        $invoiceItem->save();
                        $transferItem = new TransferItem();
                        $transferItem['transfer_id'] = $data->id;
                        $transferItem['material_id'] = \request("material_id")[$index];
                        $transferItem['quantities'] = \request("quantities")[$index];
                        $transferItem['price'] = \request("price")[$index];                                      
                        $transferItem['note'] = \request("note")[$index];                                      
                        $transferItem['invoice_item_id'] = $invoiceItem->id;                                      
                        $transferItem->save();
                }
            }
        }
        ...
        if (auth()->user()->can('browse', app($dataType->model_name))) {
            ...
            $redirect = redirect()->route("voyager.{$dataType->slug}.index", array("project_id"=>\request("project_id")));
        } else {
    ...
    public function create(Request $request)
    {
        ...
        $materials = Material::all();
        return Voyager::view($view, compact('dataType', 'dataTypeContent', 'isModelTranslatable','project','materials'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if($data->id){
            $invoice = Invoice::create([
                'num'=>$data->num,
                'project_id' => $data->tproject_id,
                'added_at' => $data->added_at,
                'photo' => $data->photo ,
                'approved' => 1               
            ]);                        
            for ($index = 0; $index < count(\request("material_id")); $index++) {
                if (\request("material_id")[$index]) {
                    if ($request->input("quantities")[$index] != 0) {
                        $invoiceItem = new InvoiceItem();
