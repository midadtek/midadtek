@if($data)
    ...
    @can ($action->getPolicy(), $data)
        <a  title="{{ $action->getTitle() }}" {!! $action->convertAttributesToHtml() !!}
            href="
            @if ($data->status == 3 || $data->status == 4)
            javascript:function() { return false; }
            @else    
            {{ $action->getRoute($dataType->name) }}
            @endif">
            <i class="{{ $action->getIcon() }}" {{ $action->getIcon() }}></i> <span class="hidden-xs hidden-sm">{{ $action->getTitle() }}</span>
        </a>       
    @endcan
