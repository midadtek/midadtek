...
<h3 style="margin-left: 600px;" >ادارة البرامج والمشاريع</h3>
		...
		@if($formType == 5)         
			...
			<h4>
			مشاريع حسب  المهندس :	<span style="color: red;">{{$userName}}</span>
			</h4>
			@endif
			@if($formType == 6)         

			<h4>
			مشاريع حسب  المدير :	<span style="color: red;">{{$managerName}}</span>
			</h4>
			@endif
			@if($formType == 7)         

			<h4>
			مشاريع حسب  الدولة :	<span style="color: red;">{{$countryName}}</span>
			</h4>
			@endif
			<h4>
		...
		<table style="width: 100%; margin-left: auto; margin-right: auto;" class="borderdtable">
			<thead>
				<tr>
					...
					<th style="text-align: center;"><strong>حالة المشروع  </strong></th>
			...
			<tbody>
				...
				<tr>
					...
					<td style=" text-align: center;">{{$project->status ? $project->getStatusDisplayAttribute():'-------'}}</td>
